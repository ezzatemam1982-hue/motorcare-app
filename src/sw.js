// MotorCare Service Worker v1.5.0
...
const CACHE_NAME = 'motorcare-cache-v1.5.0';
...
self.addEventListener('notificationclick', (event) => {
  ...
  // التأكد الحصري: كود التأجيل لا يعمل إلا إذا تم الضغط صراحة على زر التأجيل (snooze)
  if (action === 'snooze') {
    event.waitUntil(
  ...
  // في كافة الحالات الأخرى (الضغط على زر تسجيل الصيانة أو الضغط على جسم الإشعار نفسه):
  // يتم فتح التطبيق والتوجه فوراً لتبويب الصيانة وفتح نافذة تسجيل الصيانة
  event.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
