...
function processFile(filePath) {
    ...
    // 1. Remove incorrect initAuthEmailLiveWatcher inside copyAppsScriptCode
    const regexBad = /\n\s*\/\/ مراقبة حية لحقل البريد في وضع إنشاء الحساب[\s\S]*?\}\)\(\);\s*\n\s*<\/body><\/html>';/;
    if (regexBad.test(content)) {
        content = content.replace(regexBad, "\n</body></html>';");
        console.log('  [1] Cleaned bad snippet from copyAppsScriptCode.');
    } else {
        console.log('  [1] Bad snippet not found (already clean).');
    }
    ...
    if (!content.includes('async function ensureFirestoreReady')) {
        const targetMarker = 'function getCloudSyncUserKey() {';
        const ensureFirestoreCode = `        async function ensureFirestoreReady(maxWaitMs = 3500) {
        ...
        if (idx !== -1) {
            content = content.substring(0, idx) + ensureFirestoreCode + content.substring(idx);
            console.log('  [2] Added ensureFirestoreReady before getCloudSyncUserKey.');
        } else {
            console.warn('  [2] Could not find getCloudSyncUserKey marker.');
        }
    } else {
        console.log('  [2] ensureFirestoreReady already present.');
    }

    // 3. Update markAccountAsVerifiedInDB
    const markAccountMarkerStart = 'function markAccountAsVerifiedInDB(email) {';
    const markAccountMarkerEnd = 'function notifyCrossTabVerification(email) {';
    const startIdx = content.indexOf(markAccountMarkerStart);
    const endIdx = content.indexOf(markAccountMarkerEnd);

    if (startIdx !== -1 && endIdx !== -1 && startIdx < endIdx) {
        const newMarkAccountFunction = `function markAccountAsVerifiedInDB(email) {
            if (!email) return;
        ...
        `;
        content = content.substring(0, startIdx) + newMarkAccountFunction + content.substring(endIdx);
        console.log('  [3] Updated markAccountAsVerifiedInDB & added checkUrlEmailVerification.');
    } else {
        console.warn('  [3] Could not find markAccountAsVerifiedInDB boundaries.');
    }

    // 4. Update switchAuthTab and add initAuthEmailLiveWatcher in UI script section
    const oldSwitchAuthMarker = 'function switchAuthTab(mode) {';
    const googleClientIdMarker = 'const GOOGLE_OAUTH_CLIENT_ID =';
    const switchStartIdx = content.indexOf(oldSwitchAuthMarker);
    const googleClientIdx = content.indexOf(googleClientIdMarker, switchStartIdx);

    if (switchStartIdx !== -1 && googleClientIdx !== -1) {
        const newAuthTabsAndWatcher = `function switchAuthTab(mode) {
        ...
        `;
        content = content.substring(0, switchStartIdx) + newAuthTabsAndWatcher + content.substring(googleClientIdx);
        console.log('  [4] Updated switchAuthTab & added live watcher.');
    } else {
        console.warn('  [4] Could not find switchAuthTab / googleClientId boundaries.');
    }

    // 5. Replace handleAuthSubmit with findAccountByEmail, saveAccountToLocalDB, and robust handleAuthSubmit
    const authSubmitMarker = 'let _isSubmittingAuth = false;';
    const forgotModalMarker = 'function openForgotPasswordModal() {';
    const authStartIdx = content.indexOf(authSubmitMarker);
    const authEndIdx = content.indexOf(forgotModalMarker, authStartIdx);

    if (authStartIdx !== -1 && authEndIdx !== -1) {
        ...
        `;
        content = content.substring(0, authStartIdx) + newAuthSubmitCode + content.substring(authEndIdx);
        console.log('  [5] Updated handleAuthSubmit, findAccountByEmail, and saveAccountToLocalDB.');
    } else {
        console.warn('  [5] Could not find handleAuthSubmit boundaries.');
    }

    // 6. Update handleForgotPasswordStep1 and Step2
    const forgotStep1Marker = 'async function handleForgotPasswordStep1(event) {';
    const resendEmailMarker = 'function resendWelcomeAndVerificationEmail() {';
    const forgotStep1Idx = content.indexOf(forgotStep1Marker);
    const resendEmailIdx = content.indexOf(resendEmailMarker, forgotStep1Idx);

    if (forgotStep1Idx !== -1 && resendEmailIdx !== -1) {
        const newForgotStepCode = `async function handleForgotPasswordStep1(event) {
        ...
        `;
        content = content.substring(0, forgotStep1Idx) + newForgotStepCode + content.substring(resendEmailIdx);
        console.log('  [6] Updated handleForgotPasswordStep1 and Step2.');
    } else {
        console.warn('  [6] Could not find handleForgotPassword boundaries.');
    }

    // 7. Update DOMContentLoaded to initialize Firestore & live watcher
    const domLoadedMarker = "if (typeof checkUrlEmailVerification === 'function') checkUrlEmailVerification();";
    const domLoadedIdx = content.indexOf(domLoadedMarker);
    if (domLoadedIdx !== -1) {
        const domAddition = `if (typeof checkUrlEmailVerification === 'function') checkUrlEmailVerification();\n\n            // تشغيل تهيئة Firestore وقاعدة البيانات فوراً للجميع (مسجلين وزوار وواجهة الدخول)\n            if (typeof initFirestoreDatabase === 'function') {\n                try { initFirestoreDatabase(); } catch(e) {}\n            }\n            if (typeof initAuthEmailLiveWatcher === 'function') {\n                try { initAuthEmailLiveWatcher(); } catch(e) {}\n            }`;
        content = content.substring(0, domLoadedIdx) + domAddition + content.substring(domLoadedIdx + domLoadedMarker.length);
        console.log('  [7] Updated DOMContentLoaded with initFirestoreDatabase & initAuthEmailLiveWatcher.');
    } else {
        console.warn('  [7] Could not find checkUrlEmailVerification in DOMContentLoaded.');
    }
