...
print(f"Original index.html length: {len(content)} bytes")

# 1. Update serviceCentersModal Header (lines ~5209-5215)
old_header_target = '''                        <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400">124 مركز توكيل وخدمة معتمد مع ملاحة GPS الرسمية</p>
                    </div>
                </div>
                <button onclick="closeServiceCentersModal()" class="w-8 h-8 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white cursor-pointer transition-colors">
                    <i class="fa-solid fa-xmark text-sm"></i>
                </button>'''

new_header_replacement = '''                        <p class="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400"><span id="scTotalCentersHeaderCount">288</span> مركز توكيل وخدمة معتمد مع ملاحة GPS وتصحيح ذكي 🛰️</p>
                    </div>
                </div>
                <div class="flex items-center gap-2">
                    <button type="button" onclick="viewUserCorrectionsModal()" class="px-2.5 py-1 text-[11px] font-bold text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/50 hover:bg-amber-100 dark:hover:bg-amber-900/60 rounded-xl border border-amber-200 dark:border-amber-800 flex items-center gap-1.5 transition-all cursor-pointer shadow-xs" title="عرض تعديلاتي المحفوظة وتصديرها">
                        <i class="fa-solid fa-location-crosshairs text-amber-500"></i>
                        <span class="hidden sm:inline">تعديلاتي المحفوظة</span>
                        <span id="scHeaderCorrectionsBadge" class="hidden px-1.5 py-0.2 bg-amber-500 text-white rounded-full text-[9px] font-black">0</span>
                    </button>
                    <button onclick="closeServiceCentersModal()" class="w-8 h-8 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white cursor-pointer transition-colors">
                        <i class="fa-solid fa-xmark text-sm"></i>
                    </button>
...
if old_header_target in content:
    content = content.replace(old_header_target, new_header_replacement, 1)
    print("Step 1: Updated serviceCentersModal header successfully.")
else:
    # Try more relaxed matching if spacing differs
    pattern = re.compile(r'<p class="text-\[10px\] sm:text-xs text-slate-500 dark:text-slate-400">124 مركز توكيل وخدمة معتمد مع ملاحة GPS الرسمية</p>\s*</div>\s*</div>\s*<button onclick="closeServiceCentersModal\(\)"[^>]*>.*?<\/button>', re.DOTALL)
    m = pattern.search(content)
    if m:
        content = content[:m.start()] + new_header_replacement + content[m.end():]
        print("Step 1 (Regex): Updated serviceCentersModal header successfully.")
    else:
        print("Step 1 WARNING: Target for header not found!")

# 2. Update renderServiceCenters() and insert new JS controller functions
# Let's locate `function renderServiceCenters() {`
idx_render = content.find("function renderServiceCenters() {")
assert idx_render != -1, "Could not find function renderServiceCenters() in index.html"

# Find next function: `/* [UI ENHANCEMENT] وظائف القائمة المنسدلة العلوية (Top Header Hamburger Menu) */`
idx_next_func = content.find("/* [UI ENHANCEMENT] وظائف القائمة المنسدلة العلوية (Top Header Hamburger Menu) */", idx_render)
assert idx_next_func != -1, "Could not find next function anchor after renderServiceCenters"

old_render_block = content[idx_render:idx_next_func]

# New renderServiceCenters and Location Correction JS
new_render_and_correction_js = '''function renderServiceCenters() {
        ...
        async function submitLocationCorrection() {
            ...
            if (submitBtnText) submitBtnText.innerHTML = '<i class="fa-solid fa-circle-notch fa-spin me-1.5"></i> جاري الحفظ والمزامنة السحابية...';
            if (statusBox) {
                statusBox.classList.remove('hidden');
                statusBox.className = 'p-2 rounded-xl text-[11px] font-bold text-center bg-sky-50 text-sky-700 border border-sky-200 dark:bg-sky-950/60 dark:text-sky-300 dark:border-sky-800';
                statusBox.textContent = 'جاري حفظ التعديل محلياً وإرسال التنبيه للمطور...';
            }
            ...
            // 1. OFFLINE-FIRST: Save locally in SafeStorage / localStorage
            let userCorrections = {};
            try {
                const raw = (typeof SafeStorage !== 'undefined' ? SafeStorage.getItem('motorCare_scUserCorrections') : localStorage.getItem('motorCare_scUserCorrections'));
                if (raw) userCorrections = JSON.parse(raw);
            ...
            // 2. CLOUD SYNC: Save to Cloud Firestore (Collection: service_center_corrections)
            ...
            // 3. DEVELOPER EMAIL NOTIFICATION & WEBHOOK DISPATCH
            ...
            // Channel 2: EmailJS delivery if configured
            ...
            // Restore Submit UI and show success feedback
            ...
            if (statusBox) {
                ...
                statusBox.innerHTML = `
                    ...
                    <p class="text-[11px] font-normal leading-relaxed text-slate-600 dark:text-slate-300">
                        تم تفعيل الإحداثيات الجديدة على هاتفك بنجاح (Offline-First ⚡)، كما تم إرسال إشعار فوري للمطور لمراجعة التعديل واعتماده لجميع المستخدمين. شكراً لمساهمتك! 🌟
                    </p>
                `;
            }

            // Show Toast notification if showNotification function exists
...
content = content[:idx_render] + new_render_and_correction_js + content[idx_next_func:]

# Save modified index.html
with open(INDEX_PATH, 'w', encoding='utf-8') as f:
    f.write(content)

print(f"Step 2 & 3: Wrote updated index.html ({len(content)} bytes)")

# Step 4: Mirror index.html to src/index.html
shutil.copy2(INDEX_PATH, SRC_INDEX_PATH)
print("Step 4: Mirrored index.html to src/index.html")

# Verify SHA-256 parity between index.html and src/index.html
...
print(f"Verified 100% SHA-256 Parity: {h1}")

# Step 5: Bump SW Cache to v2.0.13
sw_files = [
...
for p in sw_files:
    if os.path.exists(p):
        with open(p, 'r', encoding='utf-8') as f:
            sw_c = f.read()
        sw_c_new = re.sub(r'motorcare-cache-v2\.0\.\d+', 'motorcare-cache-v2.0.13', sw_c)
        with open(p, 'w', encoding='utf-8') as f:
            f.write(sw_c_new)
        print(f"Bumped cache in {p}")
