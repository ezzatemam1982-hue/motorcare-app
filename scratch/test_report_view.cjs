...
(async () => {
    ...
    // Unhide the reportPrintSection and customPrintSection for preview
    await page.evaluate(() => {
    ...
    const secEl = await page.$('#reportPrintSection');
    if (secEl) {
        await secEl.screenshot({ path: 'scratch/screenshot_report_header.png' });
        console.log('Report header element screenshot captured!');
    }
