...
## 53. المنظومة الشاملة لعزل جلسات المستخدمين وتصفير بيانات الكراج عند التسجيل الجديد (v2.0.27)
* **التاريخ:** 24 سبتمبر 2026
* **المشكلة المبلغة من المستخدم:**
  - عند إنشاء حساب جديد، ظهرت بيانات وسيارات كراج قديمة للمستخدم السابق.
* **الأسباب الجذرية المكتشفة:**
  1. التحميل المسبق عند بدء التشغيل: حدث DOMContentLoaded كان يقوم بتحميل بيانات motorCare_AppState_v140 و IndexedDB إلى الذاكرة الحية ppState.cars حتى في حالة عدم تسجيل الدخول (motorCare_LoggedIn !== 'true').
  2. تسجيل الخروج السطحي (Shallow Logout): وجود دالة مكررة لـ handleLogout كانت تكتفي بتبديل قيمة motorCare_LoggedIn دون تصفير الذاكرة الحية ppState.cars ودون مسح مفاتيح التخزين النشطة أو مرآة IndexedDB.
  3. غياب العزل عند التسجيل الجديد: في دالة handleAuthSubmit لوضع التسجيل، لم يكن يتم استدعاء تفريغ الكراج أو عزل جلسة المستخدم.
  4. عدم إطلاق نافذة الإعداد الإلزامي: عدم استدعاء checkFirstTimeOnboarding عند تسجيل الحسابات الجديدة ذات الكراج الفارغ.

### أ. الحلول الجذرية المنفذة:
1. **محرك عزل الجلسات (detectAndIsolateUserSession):**
   - عزل تام لكل جلسة مستخدم بحسب بريده ومزود الخدمة.
   - عند التسجيل الجديد (isNewRegistration = true): تصفير فوري لـ ppState.cars = [] ومسح motorCare_AppState_v140، وجهات اتصال الطوارئ، وتوقيت المزامنة، وتفريغ مرآة IndexedDB.
   - عند التبديل بين الحسابات: حفظ حالة المستخدم السابق تلقائياً في مفتاح خاص به motorCare_AppState_<userKey> واسترجاع كراج الحساب الجديد بدقة تامة دون أي تداخل.
2. **تسجيل الخروج العميق والآمن (Deep Logout):**
   - حفظ بيانات المستخدم الحالي في مفتاحه المخصص motorCare_AppState_<userKey>.
   - إيقاف مستمع المزامنة السحابية اللحظي في Firestore فوراً لمنع أي تحديثات خلفية.
   - مسح شامل للذاكرة الحية ppState.cars = [] ومفاتيح الجلسة ومرآة IndexedDB.
   - إعادة تهيئة الواجهة لعرض الحالة الفارغة وتوجيه المستخدم لشاشة تسجيل الدخول.
3. **حماية بدء التشغيل:**
   - قصر استرجاع بيانات الكراج من localStorage و IndexedDB فقط عندما يكون motorCare_LoggedIn === 'true'.
4. **نافذة إضافة السيارة الأولى الإلزامية (checkFirstTimeOnboarding):**
   - تفتح تلقائياً بحقول فارغة عند تسجيل حساب جديد أو دخول حساب بكراج فارغ، مع إخفاء أزرار الإغلاق لمنع الوصول للتطبيق إلا بعد إدخال السيارة الأولى بنجاح.

### ب. الاختبارات والتأكيد البرمجي:
- ✅ اجتياز اختبار عزل الجلسات والتبديل المتعدد في النسخة الموديلار (scratch/test_user_session_isolation.cjs): 8 سيناريوهات كاملة بنسبة 100%.
- ✅ اجتياز اختبار المتصفح لتدفق الإعداد الأولي (scratch/verify_onboarding.cjs): 14 خطوة بدون أي خطأ بالمتصفح أو الكونسول.
- ✅ اجتياز اختبار عزل الجلسات الشامل في النسخة الموحدة index.html (scratch/test_monolithic_isolation.cjs): 100% بنجاح تام وبصفر أخطاء.
- ✅ تطابق بايت لبايت بنسبة 100% بين MotorCare-App/index.html و MotorCare-App/src/index.html.
- ✅ نجاح بناء حزمة الإنتاج Vite لنسخة motorcare-modular.
...
## 56. الدخول التلقائي المباشر بعد استعادة كلمة المرور وقبول كلمة المرور الجديدة (v2.0.31)
* **التاريخ:** 24 سبتمبر 2026
* **المشكلة:**
  1. بعد كتابة كود التحقق (OTP) وتعيين كلمة المرور الجديدة، كان التطبيق يكتفي بإغلاق النافذة وتعبئة حقول شاشة الدخول دون تسجيل الدخول التلقائي المباشر.
  2. عند محاولة الدخول لاحقاً بكلمة المرور الجديدة يدوياً، كان `firebase.auth().signInWithEmailAndPassword` يرفضها (`auth/wrong-password` أو `auth/invalid-credential`) لأن كلمة المرور في فايربيس أصلها القديم لم يُحدَّث عبر واجهة العميل بدون جلسة نشطة، مما كان يتسبب في حظر الدخول بالرغم من صحة كلمة المرور المعتمدة بالـ OTP.
* **الحلول الجذرية:**
  1. **الدخول التلقائي الفوري (`handleForgotPasswordStep2`):**
     - فور التحقق من رمز الـ OTP وتعيين كلمة المرور الجديدة، يتم حفظ الحساب محلياً وفي Firestore، وتفعيل الجلسة فوراً (`motorCare_LoggedIn = 'true'`) والدخول المباشر إلى واجهة التطبيق (`enterMainApp`) وبدء المزامنة السحابية الذكية تلقائياً.
  2. **قبول كلمة المرور الجديدة في شاشة الدخول (`handleAuthSubmit`):**
     - تم توحيد التحقق من كلمة المرور بحيث إذا كان البريد وكلمة المرور يطابقان الحساب المعتمد محلياً أو في Firestore (سواء بعد التحديث برمز OTP أو التسجيل)، يُقبل الدخول فوراً ويُعتمد الحساب دون أن يُعطله أي خطأ ناتج عن عدم تطابق كلمة المرور السابقة في Firebase Auth.
     - معالجة وإعادة تصفير متغير قفل الإرسال `_isSubmittingAuth = false` عند حالات الأخطاء لمنع تجميد زر الدخول.
  3. **ترقية كاش السيرفيس وركر:**
     - ترقية `CACHE_NAME` إلى **`motorcare-cache-v2.0.31`** ومزامنة كافة الملفات في `src/` و `dist/`.

---

## 57. سد ثغرة التخطي غير المصرح به عبر جوجل وإلغاء نافذة إدخال الإيميل اليدوية نهائياً (v2.0.32)
* **التاريخ:** 24 سبتمبر 2026
* **المشكلة المبلغ عنها:**
  - عند إلغاء أو إغلاق نافذة تسجيل الدخول بحساب Google (أو في بعض بيئات العرض)، كانت تظهر نافذة فرعية منبثقة بعنوان "تسجيل الدخول بحساب Google" (`googleAuthModal`).
  - عند كتابة أي بريد إلكتروني عشوائي في تلك النافذة والضغط على "دخول ومزامنة"، كان النظام يفتح التطبيق مباشرة دون أي تحقق من الملكية أو كلمة مرور أو رمز OTP أو مصادقة حقيقية من Google.
  - وجود ثغرة خطيرة أخرى في زر جوجل في [index.html](file:///d:/car/motorcare-modular/index.html) بسطر `else{enterMainApp();}` يدخل المستخدم في حال فشل الاستدعاء.
  - ونظراً لأن المنظومة ستتصل مستقبلاً بخدمات غير مجانية واشتراكات مدفوعة، فإن هذا التجاوز كان يمثل خطورة أمنية بالغة.
* **الإجراءات والحلول المطبقة:**
  1. **حذف نافذة `googleAuthModal` نهائياً من الـ DOM:**
     - استئصال وحذف كامل كود نافذة الـ Modal (`#googleAuthModal`) من ملف [index.html](file:///d:/car/motorcare-modular/index.html).
  2. **إزالة التخطي الالتفافي من زر Google:**
     - حذف تعبير `else{enterMainApp();}` نهائياً من زر المتابعة بجوجل ليصبح الزر يستدعي فقط دالة المصادقة الرسمية `handleSocialLogin('google')`.
  3. **تنظيف ملف [js/services/auth.js](file:///d:/car/motorcare-modular/js/services/auth.js):**
     - حذف دوال `openGoogleAuthModal` و `closeGoogleAuthModal` و `handleGoogleAuthModalSubmit` تماماً.
     - استبدال أي تحويل تلقائي للنافذة البديلة عند فشل أو إلغاء Google OAuth بتنبيه معلوماتي آمن يبقي المستخدم في شاشة الدخول ويمنع الدخول للتطبيق إلا بمصادقة أصلية وموثقة عبر خوادم Google الرسمية (`accounts.google.com`).
     - إزالة أي تصديرات للـ Window متعلقة بهذه النافذة.
  4. **ترقية كاش السيرفيس وركر إلى `motorcare-cache-v2.0.32`:**
     - تحديث `CACHE_NAME` في جميع ملفات الـ Service Worker الستة (`sw.js`, `service-worker.js`, `src/sw.js`, `src/service-worker.js`, `dist/sw.js`, `dist/service-worker.js`).
     - مزامنة كافة الملفات بين الجذر و `src/` و `dist/` وإعادة البناء عبر `npm run build`.
* **الاختبارات والتأكيد:**
  - تم إجراء اختبار أتمتة عبر Playwright Edge (`scratch/test_google_auth_security.cjs`):
    - التحقق من عدم وجود `googleAuthModal` نهائياً في شجرة الـ DOM (`false`).
    - التحقق من خلو الـ window من أي دالة نافذة التفافية (`undefined`).
    - النقر على زر Google ومحاكاة الإلغاء/الخطأ: التأكد من بقاء المستخدم داخل شاشة الدخول مع حظر الوصول للمنظومة بدون اعتماد رسمي موثق.

---

## 58. تعزيز تجربة استخدام الهواتف وحزم كاباسيتور (Mobile UX & Capacitor Essentials) والصمت الكامل عند إلغاء الدخول (v2.0.33)
* **التاريخ:** 24 سبتمبر 2026
* **المتطلبات المنجزة:**
  1. **كتم وإلغاء رسائل الإشعار المزعجة عند إلغاء جوجل:**
     - تم إزالة رسالة التوست ("تم إلغاء نافذة تسجيل الدخول عبر Google") نهائياً من [js/services/auth.js](file:///d:/car/motorcare-modular/js/services/auth.js) حتى لا تظهر أي رسالة غير مفيدة للمستخدم.
  2. **حماية واستكشاف المنصة الآمن (Platform Detection Safeguard):**
     - تم إنشاء موديول خدمات الجوال المخصص [`js/services/mobile.js`](file:///d:/car/motorcare-modular/js/services/mobile.js).
     - فحص بيئة التشغيل عبر `Capacitor.isNativePlatform()` بحراسة استثنائية `try/catch` لمنع أي أخطاء وقت التشغيل (`runtime errors`) في المتصفحات أو سطح المكتب.
  3. **هيكلية معالجة زر الرجوع الفيزيائي لأجهزة أندرويد (Hardware Back Button Hierarchy):**
     - الاستماع لحدث `backButton` عبر `@capacitor/app` مع الاستماع الاحتياطي لحدث `backbutton` العام.
     - تطبيق أولويات الرجوع الثلاث بدقة:
       * **الأولوية (أ):** في حال وجود أي نافذة منبثقة أو درج مفتوح (`Modal` أو `Drawer` أو `Popup`)، يتم إغلاق النافذة العلوية فوراً.
       * **الأولوية (ب):** في حال كان المستخدم في تبويب فرعي (بنزين، صيانة، تقارير، وثائق، فحص)، يتم الرجوع للوحة القيادة الرئيسية (`dashboard`).
       * **الأولوية (ج):** في لوحة القيادة بدون نوافذ، يتم طلب الضغط مرتين خلال ثانيتين للخروج مع إظهار توست مصغر ("اضغط مرة أخرى للخروج من التطبيق")، ثم استدعاء `App.exitApp()`.
  4. **الاهتزازات التفاعلية اللطيفة (Subtle Haptic Feedback):**
     - تكامل مع `@capacitor/haptics`:
       * نقرة خفيفة (`ImpactStyle.Light`) عند التبديل بين التبويبات ونقر الأزرار.
       * اهتزاز نجاح (`NotificationType.Success`) عند حفظ النماذج (بنزين، صيانة) ونجاح المزامنة السحابية عبر `showNotification`.
       * بديل آمن وصامت تماماً عند العمل في متصفحات الويب العادية.
  5. **دعم المساحات الآمنة (Safe Area Insets):**
     - تحديث [`css/style.css`](file:///d:/car/motorcare-modular/css/style.css) لدعم متغيرات `env(safe-area-inset-top)` و `env(safe-area-inset-bottom)`.
     - حماية الهيدر العلوي، شريط التنقل السفلي للموبايل (`mobileBottomNav`)، وحاويات المحتوى لعدم التداخل مع شريط إيماءات أندرويد (Gesture Bar) وثقب الكاميرا الأمامية (Notch).
  6. **ترقية السيرفيس وركر:**
     - ترقية `CACHE_NAME` إلى **`motorcare-cache-v2.0.33`** عبر كافة ملفات الـ SW الستة ومزامنة مجلدات `src/` و `dist/`.
* **نتائج الاختبارات:**
