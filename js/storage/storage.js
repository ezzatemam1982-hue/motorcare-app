        ...
        // المحرك المركزي الموحد لحفظ حالة التطبيق محلياً وسحابياً بأمان تام مع عزل الحسابات
        function saveAppState(triggerReason = 'general_update') {
            try {
                ...
                // حفظ نسخة معزولة خاصة بالمستخدم النشط لضمان عدم تداخل بيانات المستخدمين عند تبديل الحسابات
                try {
                    let activeUser = SafeStorage.getItem('motorCare_CurrentActiveUser');
                    if (!activeUser && typeof getCloudSyncUserKey === 'function') {
                        activeUser = getCloudSyncUserKey();
                    }
                    if (activeUser && activeUser !== 'guest') {
                        const sanitizedUser = activeUser.toLowerCase().replace(/[^a-z0-9_]/g, '_');
                        SafeStorage.setJSON('motorCare_AppState_' + sanitizedUser, appState);
                    }
                } catch(userIsoErr) {}
